          ...
          <div className="flex gap-2 md:gap-3 flex-wrap">
            {categories.map(cat => (
              <motion.button
                ...
                className={`px-3 md:px-4 py-1.5 md:py-2 rounded-full text-xs md:text-sm transition-colors ${
                  filter === cat.id
...
const PathItem = ({ item, index, total, pathRefs, animationOffset, mousePos, onSelect, getText }) => {
  ...
  const itemHeight = isPublication ? 180 : 144;
  ...
  return (
    <motion.div
      ...
      style={{
        ...
        x: useTransform(x, val => val - (isPublication ? 120 : (item.type === 'text' ? 64 : 96))),
        pointerEvents: 'auto'
      }}
    ...
    >
      <div className="relative w-full h-full">
        {item.type === 'text' ? (
          <>
            <div className="flex items-center justify-center w-32 h-full bg-neutral-100/50 rounded-md">
...
const AnimatedCanvas = memo(({ items, mousePos, onSelect, getText, isMobile }) => {
  ...
  useEffect(() => {
    let animationFrame;
    const animateLoop = () => {
      setAnimationOffset(prev => prev + 0.0005);
